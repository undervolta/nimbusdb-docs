...
:::caution
- You **can't** export `validator` and `default_value` that uses a method in the schema. If you do, those properties will be ignored.
- You can export the model relations, but you won't be able to import them back into **NimbusDB**. Please use the [Catalog Backup and Restore](/core/catalog/backup-restore) feature instead of exporting and importing each model individually.
:::
