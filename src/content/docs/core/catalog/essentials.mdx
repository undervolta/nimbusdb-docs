...
<Tabs syncKey='lang'>
    ...
    ctg_items.define("global", "shop", {     // define "shop" model inside `ctg_items` catalog
        id: {
    ...
    // ctg_items.remove_model(1);  // `shop` model has custom id of 1
    ...
    // rename model from `shop` to `storage`
    // ctg_items.rename("shop", "storage");
    ```
    </TabItem>
</Tabs>
...
<Tabs syncKey='lang'>
    ...
    var storage = ctg_items.use("storage");     // get model instance by name
    var also_storage = ctg_items.model(1);      // get model instance by custom id (`.model()` is alias of `.use()`)

    // both `storage` and `also_storage` refer to the same model instance
