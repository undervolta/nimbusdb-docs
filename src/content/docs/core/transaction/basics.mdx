...
The **ACID** properties in NimbusDB are **not** the same as ACID in relational databases due its design philosophy (in-memory and client-side first).

#### Atomicity <Badge text="Supported" variant="success" />
    
The operations in a transaction are performed on a copy of the original model. So, if one operation fails, the entire transaction will be rolled back, and the original model will remain unchanged.

#### Consistency <Badge text="Supported" variant="success" />

The schema supports constraint and validator for each column/field. Data entering the model must satisfy defined rules, or it gets rejected. So, it's supported if you're using it correctly.

#### Isolation <Badge text="Not Supported" variant="danger" />

GML is single-threaded, so technically no two transactions can run simultaneously. 

#### Durability <Badge text="Partially Supported" variant="caution" />

You need to manually save and load your data back from persistent storage to achieve data persistence (e.g using `Model.export()`/`Catalog.backup()` and `Model.import()`/`Catalog.restore()` methods).
