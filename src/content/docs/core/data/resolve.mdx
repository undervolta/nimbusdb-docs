...
## Resolving Multiple Foreign Keys
...
:::tip
For the foreign-key column in the schema definition, you must choose one data type, either `INTEGER` or `ARRAY_OF_INTEGER`. Make sure the data type and column name are consistent with the foreign-key column in the related model (e.g. `items_ids: ARRAY_OF_INTEGER` in the `shop_items` model).
:::
