...
{/*(NimbusDBColumnRules, DATA_TYPE, CONSTRAINT, validator, default)*/}
    ...
    var object_schema = {
        ...
        name: {
            ...
            validator: function(data, value) {
                return is_string(value) && string_length(value) > 0;    // name must be a non-empty string
            }
        },
        price: {
            type: [NIMBUSDB_DATA_TYPE.NUMBER, NIMBUSDB_DATA_TYPE.STRING],
...
:::tip
With combination of `OPTIONAL` constraint and `default_value`, you can create a column that is optional but will have a default value if not provided. 
...
type NimbusDBModelOptions = Partial<{
	column_order: string[];			// order of columns, default = undefined, use column_names
	custom_id: int;					// set custom ID, default = 0
	debug: boolean | 2 | 3;			// enable debugging, default = false, true/1 = ~warning, 2 = ~success, 3 = ~info
	init_cache: boolean;	        // initialize cache for initial data, default = false, true/1 = cache all, array = cache specific data by index
